...
import { refreshGoogleToken } from '@craft-agent/shared/auth'
...
const refreshFunctions: Record<string, (refreshToken: string) => Promise<{ accessToken: string; expiresAt?: number }>> = {
  google_oauth: refreshGoogleToken,
  // microsoft_oauth and slack_oauth can be added when their refresh functions are available
  // For now, they'll fall through to returning the stored token
}
...
export async function getValidOAuthToken(credentialId: CredentialId): Promise<string | null> {
  ...
  const refreshFn = refreshFunctions[credentialId.type]
  if (!refreshFn) {
  ...
  try {
    ...
    console.error(`Failed to refresh ${credentialId.type} token:`, error)
    return credential.value
  }
}
...
**4. Verify the route structure:**
  ...
  <done>
    All OAuth routes registered at /api/oauth/{provider}/{start,callback}, token refresh helper created, server starts and routes respond
  </done>
</task>
...
5. Token refresh helper:
   ...
   - Automatically refreshes expired tokens using provider-specific refresh functions
...
- Expired tokens refresh automatically without user intervention via getValidOAuthToken helper
</success_criteria>
