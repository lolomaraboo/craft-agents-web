...
must_haves:
  ...
  artifacts:
    ...
    - path: "apps/web/src/server/routes/api/oauth/index.ts"
      ...
      exports: ["oauthRoutes"]
    - path: "apps/web/src/server/lib/oauth-tokens.ts"
      provides: "Token retrieval with automatic refresh"
      exports: ["getValidOAuthToken"]
  key_links:
    ...
    - from: "apps/web/src/server/lib/oauth-tokens.ts"
      to: "@craft-agent/shared/credentials"
      via: "credential retrieval and update"
      pattern: "getCredentialManager\\(\\)\\.(get|set)"
    - from: "apps/web/src/server/lib/oauth-tokens.ts"
      to: "@craft-agent/shared/auth"
      via: "provider-specific refresh functions"
      pattern: "refresh(Google|Microsoft|Slack)Token"
---

<objective>
Implement OAuth routes for Google, Slack, and Microsoft providers with start endpoints, callback handlers, token storage, and automatic token refresh.

Purpose: Enable browser OAuth flows for all three providers. Each provider has a /start endpoint that returns an auth URL, and a /callback endpoint that exchanges the authorization code for tokens and stores them encrypted. The token helper ensures expired tokens are automatically refreshed before use.

Output: Three provider route files with start/callback handlers, token refresh helper, registered under /api/oauth/{provider}/
</objective>
...
5. Route GET /google/callback:
   ...
   - **IMPORTANT:** Store tokens using `source_oauth` credential type (not `google_oauth` which doesn't exist):
     ```typescript
     ...
     await credManager.set(
       { type: 'source_oauth', workspaceId: sessionId, sourceId: 'google' },
       {
         ...
         // Store provider info for token refresh routing
       }
     )
     ```
...
6. Export as fastify-plugin wrapped function `googleOAuthRoutes`
  ...
  <done>
    Google OAuth routes compile without errors, start generates auth URL, callback exchanges code for tokens using source_oauth credential type
  </done>
</task>
...
3. Route GET /slack/callback:
   ...
   - **IMPORTANT:** Store with `source_oauth` type:
     ```typescript
     await credManager.set(
       { type: 'source_oauth', workspaceId: sessionId, sourceId: 'slack' },
       { value: tokens.accessToken, refreshToken: tokens.refreshToken, expiresAt: tokens.expiresAt }
     )
     ```
...
Export both as fastify-plugin wrapped functions.
  ...
  <done>
    Slack and Microsoft OAuth routes compile, handle provider-specific auth flows, use source_oauth credential type
  </done>
</task>

<task type="auto">
  <name>Task 3: Create automatic token refresh helper and OAuth route index</name>
  <files>
    apps/web/src/server/lib/oauth-tokens.ts
    apps/web/src/server/routes/api/oauth/index.ts
    apps/web/src/server/routes/api/index.ts
  ...
  <action>
**1. Create `apps/web/src/server/lib/oauth-tokens.ts`:**

This helper provides automatic token refresh when retrieving OAuth tokens:
...
/**
 * Provider-specific refresh functions keyed by sourceId
 * Returns new accessToken and optional expiresAt
 */
const refreshFunctions: Record<string, (refreshToken: string) => Promise<{ accessToken: string; expiresAt?: number }>> = {
...
/**
 * Get a valid OAuth token, automatically refreshing if expired or about to expire.
 *
 * @param credentialId - The credential ID to retrieve (must be source_oauth type)
 * @returns The valid access token, or null if not found or refresh failed
 */
export async function getValidOAuthToken(credentialId: CredentialId): Promise<string | null> {
  const credManager = getCredentialManager()
  const credential = await credManager.get(credentialId)
  ...
  // Token needs refresh - check if we have a refresh token and refresh function
  if (!credential.refreshToken) {
    // No refresh token available - return current token (may be expired)
    return credential.value
  }

  // For source_oauth credentials, use sourceId to determine provider
  const provider = credentialId.sourceId
  const refreshFn = provider ? refreshFunctions[provider] : undefined

  if (!refreshFn) {
    // No refresh function for this provider - return current token
    return credential.value
  }

  try {
    // Attempt to refresh the token
    const refreshed = await refreshFn(credential.refreshToken)

    // Update stored credential with new token
    await credManager.set(credentialId, {
      ...credential,
      value: refreshed.accessToken,
      expiresAt: refreshed.expiresAt,
      // Keep the existing refreshToken (Google doesn't return a new one on refresh)
    })

    return refreshed.accessToken
  } catch (error) {
    // Refresh failed - return current token (caller can handle 401 if truly expired)
...
/**
 * Check if a credential needs refresh (for informational purposes)
 */
...
**2. Create `apps/web/src/server/routes/api/oauth/index.ts`:**
...
**3. Update `apps/web/src/server/routes/api/index.ts`:**
...
**4. Verify the route structure:**
  ...
  <done>
    All OAuth routes registered at /api/oauth/{provider}/{start,callback}, token refresh helper created with Google/Microsoft/Slack support, server starts and routes respond
  </done>
</task>
...
4. Each provider's /callback endpoint:
   ...
   - Stores tokens in credential manager using source_oauth type
   - Returns HTML success/error page
5. Token refresh helper:
   - Returns valid token directly when not expired
   - Automatically refreshes expired tokens using provider-specific refresh functions (google, microsoft, slack)
   - Updates stored credentials after successful refresh
</verification>
...
- Tokens stored encrypted via credential manager using `source_oauth` type with sourceId identifying provider
- Slack uses HTTPS relay pattern for redirect URI
- HTML response pages for callback success/failure
- Expired tokens refresh automatically via getValidOAuthToken helper for all three providers (google, microsoft, slack)
</success_criteria>
