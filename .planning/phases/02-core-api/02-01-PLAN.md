...
- Methods: getSessions(), getSession(id), createSession(workspaceId, options), deleteSession(id), sendMessage(sessionId, message, ...), cancelProcessing(sessionId)
- For now, implement stubs that log and return mock data - actual integration with @craft-agent/shared comes in Phase 3 when we have the event streaming layer

Why stubs: The real SessionManager needs event callbacks for streaming responses. We'll wire those up in Phase 3 (Real-time Events). For Phase 2, we prove the HTTP layer works.
  </action>
  <verify>
```bash
cd /opt/craft-agents-web && bun run tsc --noEmit -p apps/web/tsconfig.json
